...
| **Layer 4: Device & Account Hardening** | Multiple fake accounts on 1 phone | Bind active challenge enrollment to hardware `DeviceUUID` and unique Health Account ID. Disallow switching Baktas user accounts on a single physical device during an active challenge. |
...
Baktas aggregates health data from three primary APIs:
