...
- **Design Tokens**: M3 `ColorScheme` roles (`scheme.primary` `#10B981`/`#34D399`, `scheme.surface`, `scheme.primaryContainer`, `scheme.inverseSurface`, `scheme.tertiary`), `Theme.of(context).textTheme.*` for standard text styles (reserving `BaktazType` strictly for display styles: `displayHero`, `metricHero`, `headlineTitle`, `subheadingUppercase`, `labelRanking`), `BaktazSpacing.*` and `BaktazRadius.*` tokens.
- **Shared UI Components**: `BaktazAvatar`, `BaktazCard`, `BaktazButton`, `BaktazListRow`, `BaktazStatusBadge`, `BaktazTextField`, `BaktazSectionHeader`, `BaktazDivider`, `ConfirmationDialog`, `StatSubCard`, `IdentityBlock`.
- **Localization**: All visible text strings are localized via `context.i18n.account.*` keys.
...
#### Key Components:
- **`AccountHeaderCard`**: Custom widget displaying `BaktazAvatar` (64px), full name (`Theme.of(context).textTheme.headlineMedium`), username (`Theme.of(context).textTheme.bodyMedium`), and member since date. Includes "Edit Profile" button routing to `/account/profile`.
...
### 2.4 `ManagePaymentScreen` (`/account/payment`) — Payment & Payout Management

Manages saved payment methods and payout destinations for host monetization. 
**Specification Source:** `docs/superpowers/specs/manage_payment/2026-08-30-00-overview.md`
      ...
      child: Padding(
        ...
        child: Row(
          children: [
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  BaktazText(
                    text: isHostTier
                        ? context.i18n.host.premium_host
                        : context.i18n.host.regular_user,
            ...
            BaktazButton(
              label: isHostTier
                  ? context.i18n.host.manage
                  : context.i18n.host.upgrade,
        ...
        itemBuilder: (context, index) {
          ...
          return AccountContentTile(
            ...
            trailing: optionKey == 'stepsSync' && isStepsSyncActive
                ? BaktazStatusBadge(
                    label: context.i18n.steps.sync_active,
  ...
  Widget build(BuildContext context) => Skeletonizer(
    ...
    child: Row(
      children: [
            ...
            children: [
              Row(
                children: [
                  ...
                  if (isHostTier)
                    BaktazStatusBadge(
                      label: context.i18n.host.premium_host,
        ...
        const SizedBox(height: BaktazSpacing.md),
      ],
    ),
  );
}
...
class _InlineStatsRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Skeletonizer(
    enabled: isLoading,
    child: Padding(
      padding: const EdgeInsets.symmetric(horizontal: BaktazSpacing.md, vertical: BaktazSpacing.md),
      child: Row(
        mainAxisAlignment: MainAxisAlignment.spaceAround,
        children: [
          _InlineStat(
            value: MoneyFormatter.formatWithoutSymbol(steps.toDouble()),
            label: context.i18n.account.challenge_steps,
          ),
          Text('·',
...
class _Variation2GroupedList extends StatelessWidget {
  ...
  /// Maps raw option keys to display labels via i18n.
  /// Fallback: PascalCase split of the key itself.
  String _optionLabel(BuildContext context, String key) {
    // In production, use context.i18n.profile.<key> getters.
