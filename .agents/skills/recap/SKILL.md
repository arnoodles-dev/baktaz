---
name: recap
description: Summarize the last N agent sessions for the current project, grouped by date, with highlight observations per session. Use when the user asks "recap", "what have we been doing", "today", "this week", "session history", "past sessions", or wants a rollup of previous work.
argument-hint: "[last N | today | this week]"
user-invocable: true
---

The user wants a recap or session overview. Time window args: $ARGUMENTS
...
### Timeline mode (quick overview)

For a quick timeline without deep recall:
